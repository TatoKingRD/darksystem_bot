...
const { tekrarlaBaslat, tekrarlaDurdur, gorevleriListele } = require('../commands/tekrarla');
...
module.exports = async function messageHandler(client, message) {
  ...
  if (message.content.startsWith('!tekrarla')) {
    return tekrarlaBaslat(message);
  }

  if (message.content.startsWith('!durdur')) {
    return tekrarlaDurdur(message);
  }

  if (message.content === '!gorevler') {
    return gorevleriListele(message);
  }
